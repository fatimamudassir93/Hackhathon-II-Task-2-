"use client";

import { ToolCallInfo } from "@/lib/chat-api";

interface ToolCallDisplayProps {
  toolCalls: ToolCallInfo[];
}

export default function ToolCallDisplay({ toolCalls }: ToolCallDisplayProps) {
  if (!toolCalls || toolCalls.length === 0) return null;

  return (
    <div className="mt-2 space-y-1">
      {toolCalls.map((tc, i) => (
        <div
          key={i}
          className="flex items-center gap-2 text-xs text-gray-400 glass px-3 py-1.5 rounded-lg animate-scale-in"
          style={{ animationDelay: `${i * 0.05}s` }}
        >
          <svg className="w-3 h-3 text-purple-400 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
              d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.066 2.573c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.573 1.066c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.066-2.573c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" />
            <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
          </svg>
          <span className="text-purple-300 font-mono">{tc.tool}</span>
          {tc.result && "status" in tc.result && (
            <span className="text-green-400">
              {String(tc.result.status)}
            </span>
          )}
          {tc.result && "error" in tc.result && (
            <span className="text-red-400">
              {String(tc.result.error)}
            </span>
          )}
        </div>
      ))}
    </div>
  );
}
